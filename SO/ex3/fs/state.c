...
void inode_table_init() {
    for (int i = 0; i < INODE_TABLE_SIZE; i++) {
        ...
        inode_table[i].lock = malloc(sizeof(pthread_rwlock_t));
        if(pthread_rwlock_init(inode_table[i].lock, NULL) != 0) {
            fprintf(stderr, "inode_table_init: error trying to init node's lock\n");
            exit(EXIT_FAILURE);
        }
    }
}
    ...
    for (int inumber = 0; inumber < INODE_TABLE_SIZE; inumber++) {
        ...
        if (inode_table[inumber].nodeType == T_NONE) {
            ...
            if(pthread_rwlock_unlock(inode_table[inumber].lock) != 0) {
                printf("inode_create: error unlocking node %d\n", inumber);
                exit(EXIT_FAILURE);
            }
            return inumber;
        }
...
/*
 * returns pointer to node's lock
 */
